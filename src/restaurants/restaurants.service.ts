...
export class RestaurantService {
  ...
  async allRestaurants({ page }: RestaurantsInput): Promise<RestaurantsOutput> {
    try {
      ...
      return {
        ok: true,
        results: restaurants,
        totalPages: Math.ceil(totalResults / itemsInPage),
        totalResults,
      }
    } catch (error) {
      return {
  ...
  async editDish(
    ...
    try {
      ...
      if (owner.id !== dish.restaurant.ownerId) {
        return {
          ok: false,
          error: "You can't edit a dish that you dont own",
        }
      }
  ...
  async deleteDish(
    ...
    try {
      ...
      if (owner.id !== dish.restaurant.ownerId) {
        return {
          ok: false,
          error: "You can't delete a dish that you dont own",
        }
      }
  ...
  async myRestaurant(
    ...
    try {
      const restaurant: any = await this.restaurants.findOne({
        where: { id },
